...
public class CityCreateValidator : AbstractValidator<CityCreateModel>
{
    ...
    {
        RuleFor(x => x.Name).NotEmpty().MinimumLength(3).WithMessage("Name must be at least 3 characters long")
            .MaximumLength(150).WithMessage("Name must be between 3 and 100 characters").DependentRules(() =>
            {
                RuleFor(x => x.Name)
        ...
        RuleFor(x => x.Slug).NotEmpty().MinimumLength(3).WithMessage("Slug must be at least 3 characters long")
            .MaximumLength(150).WithMessage("Slug must be between 3 and 100 characters");
        RuleFor(x => x.Description).NotEmpty().MinimumLength(3).WithMessage("Slug must be at least 3 characters long")
            .MaximumLength(500).WithMessage("Slug must be between 3 and 100 characters");
        RuleFor(x => x.CountryId)
            .NotEmpty().WithMessage("CountryId must be provided")
